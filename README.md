...
This is walk through for an .NET Authorization Lab, now updated for .NET 6 and VS2022.

Step 0: Preparation
===================

Create a new, blank, .NET project.
-------------------------------------

* File > New > Project > ASP .NET Core Web App
* Select ASP .NET Core Web App, give the project a name of `AuthorizationLab` and click Next.
* Select .NET 6.0 for Framework, Authentication Type as None, Configuration for HTTPS checked, Enable Docker Unchecked. Do not use top-level statements checked.
* click Create
* It takes a few seconds and .NET generated a web app for you already. You may click on the green triangle on top of VS to launch the app, and see your homepage at https://localhost:7241/


Add MVC to the app. 
-------------------

* Edit `Program.cs`  and add `builder.Services.AddMvc(); before the line of AddRazorPages(), and comment out AddRazorPages().
* comment out app.MapRazorPages();
* Where what you just commented out, add the following code to setup MVC default routing;
...
* Create a `Controllers` folder. Naming here is very strict, since .NET uses a convention-based approach where it expects certain folder structures and naming patterns to automatically map controllers, views, and models.
...
* In ASP.NET Core 2.1 the `Microsoft.AspNetCore.Authentication` package contains all the authentication and authorization packages, so you don't need to add any extra packages or references.
* Open `Program.cs`
* Add `app.UseAuthentication();` after the `builder.Build()` method.
* Add Cookie middleware to the authentication service by adding the following after `builder.Services.AddMvc()` method.
...
* Change the `AddMvc()` call in `Program.cs` to add a default authorization policy to the MVC configuration.
...
* Run and get the error. Why? You've made every page require authentication. This even includes the login pages. 
...
=======================
* Return to `Program.cs` and locate the `builder.Services.AddAuthentication()` call.
* After `builder.Services.AddAuthentication()` add a call to `builder.Services.AddAuthorization()` and create a simple policy as shown below.
...
* Next create a policy for the requirement, registering it in the `ConfigureServices()` in `Program.cs`, inside the authorization configuration.
...
* Open `Program.cs`, and register the handlers in the DI container by adding the following. Note that they don't have to be singletons, you can use the DI system to inject constructor parameters into handlers, so, for example, if you're injecting an EF repository you may want to add your handler as scoped to a request.

```c#
builder.Services.AddSingleton<IAuthorizationHandler, HasBadgeHandler>();
builder.Services.AddSingleton<IAuthorizationHandler, HasTemporaryPassHandler>();
```
...
* Finally register the document repository in the services collection in `Program.cs`
